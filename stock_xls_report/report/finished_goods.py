...
class StockReportXls(models.AbstractModel):
    _name = 'report.stock_xls_report.stock_report_xls'
...
#                     or prod.location_dest_id.usage =='view' or prod.location_dest_id.usage =='procurement' or prod.location_dest_id.usage =='transit'
            ...
            final_transfer1 = ob_transfer_two - ob_transfer   
            opening_bal= ob_purchase - ob_sale + ob_inventory_loss1 + final_transfer1 + ob_sale_return - ob_purchase_return + ob_production - ob_itp
            final_transfer = transfer_two - transfer
            closing_bal = opening_bal + purchase + production + final_transfer + inventory_loss1 - inventory_loss - itp - sale
#                 if 'MO' in prod.name:
            if check:
                vals = {
                    ...
                    'closing_bal': closing_bal*cost
                                   # + ' ' + str(product.attribute_value_ids.name or ' ')
                }
            else:
                vals = {
                        'code': product.default_code or ' ',
